...
class EnsembleGenerator:
    ...
    @classmethod
    def load_from_file(cls, filepath) -> EnsembleGenerator:
        """
        Load an ensemble from an NPZ file generated by an EnsembleGenerator.

        Parameters
        ----------
        filepath : str
            Path of the npz file; specifying the extension is not necessary.

        Returns
        -------
        ensemble : EnsembleGenerator
            EnsembleGenerator object containing all the data on the loaded ensemble.
        """
        path = Path(filepath).with_suffix(".npz")
        data = np.load(path)
        ensemble = cls(
            temperature=duq.Quantity(data["temperature_pressure"][0], duq.Unit(data["unit_temperature"])),
            pressure=duq.Quantity(data["temperature_pressure"][1], duq.Unit(data["unit_pressure"])),
            unit_length=duq.Unit(data["unit_length"]),
            unit_time=duq.Unit(data["unit_time"]),
        )
        ...
        #
        # # prepare data for inputting into force field
        ...
        ensemble._positions = data["positions"]
        ensemble._velocities = data["velocities"]
        ensemble._box_coordinates = data["box_coordinates"]
        ensemble._atomic_numbers = data["atomic_numbers"]
        ensemble._molecule_ids = data["molecule_ids"]
        ensemble._connectivity_matrix = data["connectivity_matrix"]
        ensemble._num_molecules_total = np.unique(ensemble._molecule_ids).size
        ensemble._num_atoms_total = ensemble.atomic_numbers.size
        ensemble._is_2d = True if ensemble.positions.shape[1] == 2 else False
        return ensemble
