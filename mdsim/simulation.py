...
class MDSimulation:
    ...
    def load_initial_values_from_file(self, filepath) -> None:
        """
        Load initial values from an '.npz' file generated by a subclass of
        ...
        -------
            None
            Initial values are loaded into the current object.
        """

        path = Path(filepath).with_suffix(".npz")
        init_vals = np.load(path)
        self._init_positions = init_vals["positions"]
        ...
        self._forcefield.fit_to_input_data(
            self._num_molecules,
            self._num_atoms,
